...
class DigitNeuralNet():
    ...
    def train(self, training_data, labels):
        ...
        for i in range(len(self.layer_sizes) - 1):
            ...
            self.weights.append(np.random.randn(self.layer_sizes[i], self.layer_sizes[i+1]) * 0.15)
            self.biases.append(np.zeros((1, self.layer_sizes[i+1])))

        for epoch in range(self.epochs):
            acts, zs = self.forward_propagation(training_data)
            current_cost = cost_CCE(y, acts[-1]) # acts[-1] would be the output vector
        ...
        return cost_CCE(y, acts[-1]) # Final cost
